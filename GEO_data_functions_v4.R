...
# Retrieve data from NCBI GEO using GSE ID
# Unzip the matrix and annotation files
downloadGEO = function(GSE){
	directoryStr = paste("./Data/",GSE,sep = "")
	dir.create(directoryStr)
	setwd(directoryStr)
	inDir = getwd()
	getGEO(GEO = GSE, filename = NULL, destdir = inDir, GSElimits = NULL, GSEMatrix = TRUE, AnnotGPL = TRUE, parseCharacteristics = TRUE,getGPL = TRUE)
	print(getwd())
...
# Load the series matrix data alone from the series_matrix.txt file
# Look for "!series_matrix_table_begin"
...
# Load the annotation data from the .annot file
...
# Count samples and return their names from the !Sample_title entry in the series_matrix.txt file
